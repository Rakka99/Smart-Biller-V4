...
private class MainViewModel {
    ...
    var customers by mutableStateOf(emptyList<id.smartbiller.app.data.Customer>())
    var billings by mutableStateOf(emptyList<id.smartbiller.app.data.Billing>())
    ...
    fun search(query: String) {
        ...
        customers = customers.filter {
            it.customerNo.contains(query, true) ||
                (it.name ?: "").contains(query, true) ||
                (it.address ?: "").contains(query, true)
        }
    }
    fun checkInquiry(customerNo: String) {
        val customer = customers.firstOrNull { it.customerNo == customerNo } ?: return
        val billing = billings.firstOrNull { it.customer.customerNo == customerNo } ?: return
        inquiry = id.smartbiller.app.data.InquiryResponse(
            customer,
            id.smartbiller.app.data.InquiryBilling(billing.period, billing.total, billing.total, 2500.0),
            id.smartbiller.app.data.Inquiry("INQ-$customerNo", "READY")
        )
    }

    private fun loadDemoData() {
        customers = listOf(
            id.smartbiller.app.data.Customer("1", "535111194993", "Pelanggan Demo 01", "123456789", "Sumedang", id.smartbiller.app.data.ULP("ULP Sumedang"), -6.8587, 107.9236),
            id.smartbiller.app.data.Customer("2", "535111194994", "Pelanggan Demo 02", "123456790", "Tanjungsari", id.smartbiller.app.data.ULP("ULP Sumedang"), -6.952, 107.826)
        )
        billings = customers.mapIndexed { i, c ->
            id.smartbiller.app.data.Billing("B$i", "2026-08", "PREVENTIF", if (i == 0) "PAID" else "UNPAID", 175000.0 + i * 25000, "2026-08-20", c)
        }
        summary = id.smartbiller.app.data.BillingSummary("2026-08", "PREVENTIF", customers.size, 0, 0)
        lastSync = "Sinkronisasi demo berhasil"
    }
}

@Composable private fun AppBackground(content: @Composable () -> Unit) {
    Box(Modifier.fillMaxSize().background(Color(0xFF071A33))) { content() }
}

@Composable private fun GlassCard(
    modifier: Modifier = Modifier,
    content: @Composable ColumnScope.() -> Unit
) {
    Column(
        modifier
            .background(Color.White.copy(alpha = 0.10f), MaterialTheme.shapes.large)
            .padding(18.dp),
        content = content
    )
}
    ...
    AppBackground {
        Column(Modifier.fillMaxSize().padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
            GlassCard(Modifier.fillMaxWidth()) {
                ...
                OutlinedTextField(value = password, onValueChange = { password = it }, label = { Text("Password") }, modifier = Modifier.fillMaxWidth(), singleLine = true, visualTransformation = if (visible) VisualTransformation.None else PasswordVisualTransformation(), trailingIcon = { TextButton(onClick = { visible = !visible }) { Text(if (visible) "Sembunyikan" else "Lihat") } })
                vm.error?.let { err -> Spacer(Modifier.height(8.dp)); Text(err, color = Color(0xFFFFB4AB)) }
                Spacer(Modifier.height(14.dp))
                Button(onClick = { vm.login(username, password) }, enabled = !vm.loading, modifier = Modifier.fillMaxWidth()) { Text(if (vm.loading) "Memeriksa..." else "Masuk") }
        ...
        Scaffold(containerColor = Color.Transparent, bottomBar = {
            NavigationBar {
                items.forEachIndexed { index, item ->
                    NavigationBarItem(selected = tab == index, onClick = { tab = index }, icon = { Icon(item.second.second, item.second.first) }, label = { Text(item.second.first) })
                }
            }
        }) { padding ->
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        item {
            GlassCard(Modifier.fillMaxWidth()) {
                ...
                Text(vm.lastSync, color = Color.White.copy(alpha = 0.58f), style = MaterialTheme.typography.bodySmall)
            }
        }
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                MetricCard("Pelanggan", (vm.summary?.preventif ?: 0).toString(), Modifier.weight(1f))
                MetricCard("Bayar", vm.billings.count { it.status == "PAID" }.toString(), Modifier.weight(1f))
                MetricCard("Belum", vm.billings.count { it.status != "PAID" }.toString(), Modifier.weight(1f))
            }
        }
        item {
            GlassCard(Modifier.fillMaxWidth()) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Default.Map, null, tint = Color(0xFFFFD54F), modifier = Modifier.size(32.dp))
                    Spacer(Modifier.width(10.dp))
                    Column {
                        Text("Peta Pelanggan", color = Color.White, fontWeight = FontWeight.Bold)
                        Text("Lokasi pelanggan tersedia melalui Google Maps", color = Color.White.copy(alpha = 0.65f), style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
        }
...
@Composable private fun MetricCard(title: String, value: String, modifier: Modifier = Modifier) {
    GlassCard(modifier.fillMaxWidth()) {
        Text(title, color = Color.White.copy(alpha = 0.65f), style = MaterialTheme.typography.bodySmall)
        Spacer(Modifier.height(6.dp))
        Text(value, color = Color.White, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
    }
}

@Composable private fun CustomerScreen(vm: MainViewModel) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        item { OutlinedTextField(value = query, onValueChange = { query = it }, modifier = Modifier.fillMaxWidth(), singleLine = true, label = { Text("Cari ID / nama / alamat") }) }
...
@Composable private fun BillingScreen(vm: MainViewModel) {
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
...
@Composable private fun InquiryScreen(vm: MainViewModel) {
    var customerNo by rememberSaveable { mutableStateOf("535111194993") }
