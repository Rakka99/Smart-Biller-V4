...
function App(){
  ...
  const [customerNo,setCustomerNo]=useState(""),[inquiry,setInquiry]=useState<any>(null);
  ...
  async function inquiry(){setBusy(true);setError("");try{const {data}=await api.post("/pln/inquiry",{customerNo});setInquiry(data);await refresh()}catch(e:any){setError(e.response?.data?.error||"Inquiry gagal")}finally{setBusy(false)}}
  async function pay(){if(!inquiry?.inquiry?.id||!confirm("Lanjutkan pembayaran?"))return;setBusy(true);try{await api.post("/payments",{inquiryId:inquiry.inquiry.id});await refresh();alert("Pembayaran diproses")}catch(e:any){setError(e.response?.data?.error||"Pembayaran gagal")}finally{setBusy(false)}}
  ...
  return <main className="app">
    ...
    {tab==="pln"&&<section className="section"><h2>Cek Tagihan PLN</h2><div className="search-box"><input value={customerNo} onChange={e=>setCustomerNo(e.target.value.replace(/\D/g,""))} placeholder="ID Pelanggan"/><button disabled={busy||customerNo.length<6} onClick={inquiry}>{busy?"...":"Cek"}</button></div>{inquiry?.customer&&<div className="invoice-card"><h3>{inquiry.customer.name||"Pelanggan"}</h3><p>{inquiry.customer.customerNo} · Meter {inquiry.customer.meterNo||"—"}</p>{inquiry.billing&&<><div className="total">{money(inquiry.billing.selling_price)}</div><p>Periode {inquiry.billing.periode||"—"}</p></>}<button onClick={pay}>Bayar Sekarang</button></div>}</section>}
